...
1. 記録詳細の比較導線は、保存写真の枚数ではなく **解析完了済みの比較可能な写真が2枚以上あるか** で表示可否を判定する必要があります。
2. 比較画面で同じ写真を左右に選んだ場合は、注意表示だけでなく、変化のメッセージと項目別点差を表示しないようにする必要があります。
3. 撮影画面は、写真を選択した後に取り込み前の操作ボタンが重複して残らず、「撮り直す／別の写真を選ぶ／この写真で振り返る」が明確になる状態を確認する必要があります。
4. 2回分の解析完了結果が揃った結果画面から、利用者が迷わず比較へ進める導線を設けるか、チームで最終判断する必要があります。
